...
jQuery(document).ready(function($) {
  ...
  メモ。2025-04-18
  setTimeout(function() {
    $('body').append('<p style="color: green;">すべての表示が終了しました！</p>');
  }, 5000);
  setTimeout(function() {
  $.ajax({
      url: 'process.php', // PHPファイルのパス
      type: 'POST', // メソッドをPOSTに設定
      data: {}, // 送信するデータ（必要に応じて）
      success: function(response) {
          // PHPのレスポンスを画面に表示
          $('body').append('<p style="color: green;">' + response.message + '</p>');
      },
      error: function() {
          $('body').append('<p style="color: red;">エラーが発生しました。</p>');
      }
  });
  }, 5000); // 5秒後に処理を開始
  */
